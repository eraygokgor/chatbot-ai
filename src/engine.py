...
import pickle
from pipeline.cluster import ClusterIntents
from pipeline.eda import ExploreData
from pipeline.evaluation import Eval
from pipeline.predict import Predict
from pipeline.prepare import PrepareData
from pipeline.train import ModelTrain
from pipeline.utils import parse_files, parse_data_csv
...
if args.cluster:
    if os.path.isdir(DATA_DIR):
        ...
        if not preprocessed_data.empty:
            ...
            # cluster intents
            clustered_intents = ClusterIntents(preprocessed_data, LABEL_FILE, MODULE_URL,
                                               sorted_labels=False)  # if sorted_labels is True, labelled data sorted
            # by intent
